...
行動中のエネルギー消費量 (kcal) =
  (1.8 * 行動時間（h）） + 
  （（0.3 * 歩行距離（km）） + （10.0 * 上りの累積標高差（km）） + （0.6 * 下りの累積標高差（km））） * （体重（kg） + 装備の重量（kg））） 
```
...
type Mountain = {
  ...
  elevationGainUp: number;   // 上りの累積標高差 (km)
  elevationGainDown: number; // 下りの累積標高差 (km)
  distance: number;          // 歩行距離 (km)
  duration: number;          // 標準コースタイム (h)
};
```
